...
- [ ] Write a failing `BodyContextTest` proving every known key is present, a partial input normalizes missing keys to `UNKNOWN`, the input map is defensively copied, and `UserProfile()` defaults to all `UNKNOWN`.
- [ ] Run `./gradlew testDebugUnitTest --tests '*BodyContextTest' --no-daemon` and confirm compilation fails because the types do not exist.
- [ ] Implement the enum names verbatim and a normalized immutable value object whose lookup can never treat absence as favorable.
- [ ] Add the conservative `UserProfile` default.
- [ ] Re-run the focused test and commit `feat: add movement capability domain model`.
...
- [ ] Write failing codec tests for complete round-trip, partial maps, malformed JSON, nested/non-string values, oversized input, unknown keys, unknown values, duplicate keys, and deterministic allowlisted output.
- [ ] Run the codec test and confirm it fails because the codec is absent.
- [ ] Implement a maximum-4096-character flat JSON object parser that catches only its own expected decode exception; make every corrupt case return all `UNKNOWN` without including payloads in errors.
- [ ] Write failing repository tests proving round-trip, fresh-repository reload, unrelated-field preservation, and one revision increment.
- [ ] Map the entity column through the codec and validate the normalized in-memory field before the existing atomic save.
- [ ] Re-run focused codec/repository tests and commit `feat: persist movement capabilities locally`.
...
- [ ] Write a failing real-version-7 migration test that seeds every profile field plus template/session/set history, migrates, verifies all values remain unchanged, verifies capability JSON resolves to all `UNKNOWN`, and runs `PRAGMA foreign_key_check`.
- [ ] Write failing chain tests that open manual schemas at versions 1 through 7 with the applicable full migration chain and verify schema 8 is reached.
- [ ] Bump Room to 8, add one `TEXT NOT NULL DEFAULT '{}'` profile column, register 7→8 in production and all existing test chains, and keep destructive fallback absent.
- [ ] Add a disk-backed repository close/reopen test proving capability reload after database recreation.
- [ ] Run `connectedDebugAndroidTest --no-daemon`, verify nonzero test execution, and commit `feat: migrate profiles to capability schema`.
...
- [ ] Write failing ViewModel tests for completion gating, explicit `UNKNOWN`, back/forward retention, and SavedState recreation preserving answered keys without converting unanswered keys to `UNKNOWN`.
- [ ] Run the focused ViewModel test and confirm the expected failures.
- [ ] Add saved-state serialization using allowlisted enum names, validate the first unanswered capability, and include normalized capability data in the existing one-save onboarding completion.
- [ ] Build a scrollable resource-backed step and shared radio-like controls with capability label, description, option label, selected state, minimum touch target, and deterministic focus order semantics.
- [ ] Add an instrumentation semantics test that selects explicit “Not sure” and observes its selected state.
- [ ] Run focused JVM and connected UI tests and commit `feat: collect movement preferences in onboarding`.
...
- [ ] Write failing Profile tests proving persisted values load, edits remain draft-only, cancel does not save, save performs one whole-profile write, unrelated fields survive, and revision semantics remain repository-owned.
- [ ] Run the focused Profile test and confirm it fails because editing is absent.
- [ ] Implement draft editing, static non-medical copy, BackHandler cancellation, and one atomic save of the newest current profile plus the draft capability value.
- [ ] Write a failing planner regression test using two fresh planner instances and otherwise-identical contexts with opposite capability values.
- [ ] Run it, confirm the missing domain integration failure first, then preserve identical workout output without adding any planner branch.
- [ ] Run Profile/planner tests and commit `feat: edit movement preferences from profile`.
...
- [ ] Rewrite the merged body-aware documents so this milestone is capability-only and mark measurements, metadata, eligibility/ranking, substitutions, history evidence, progression, and deload explicitly out of scope.
- [ ] Document seven fields, four answers, local Room flow, fresh onboarding, existing-user migration, existing backup inclusion, absent export/delete subsystem as an out-of-scope roadmap item, unchanged planner behavior, and the next reviewed-metadata/eligibility milestone.
